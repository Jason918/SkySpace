...
//	private final String node_ip;
	 ...
	 * @param tupleSpace 
	 */
	public void startRequestListener(final ENV tupleSpace) {
		request_listener = new Thread(new Runnable() {
			public void run() {
				...
				if (!group.isMulticastAddress()){
					ENV.logger.severe("group is not multicast address");
					return;
				}
		        ...
		        try {  
		            ...
		            ENV.logger.fine("Request listener is working");
		            while (true && !request_listener_stop) {
		                DatagramPacket dp = new DatagramPacket(buffer, buffer.length);   
		                ms.receive(dp);//TODO 可能会有丢包???
		                String s = new String(dp.getData(),0,dp.getLength());
		                ENV.logger.info("M-listener GOT packet:" + s);
		                if (request_listener_stop) break;
		                try {
		                	JSONObject jo = new JSONObject(s);
		                	tupleSpace.handleRequest(new EnvGroup(jo));
		                } catch(JSONException e) {
		                	ENV.logger.warning("Not JSON format:" + s);
		                }
	 ...
	 * 		format: {op = "send result",EnvGroup eg,EnvItem ei,boolean isFromOwner}.
	 ...
	 * 		format: {op = "acquire EnvItem",EnvItem ei,EnvGroup eg}
	 * 		*此时要考虑 跟多播线程的数据同步问题。由TuplePool来保证线程安全。
	 * 		返回：{boolean roger, boolean result,EnvItem ei,String message}
	 * 		如果result为true，表示允许对方acquire EnvItem，对方需要返回是否接受该EnvItem。格式同上。
	 * 		如果result为false，则可以在message中说明原因。
	 * 		
	 * 		
	 * @param tupleSpace 
	 */
	public void startResponseListener(final ENV tupleSpace){
			...
			public void run() {
				try {
					...
					while (!result_listener_stop) {
						...
						ENV.logger.info("S-listener received:"+data);
						
						JSONObject obj ;
						try {
							obj = new JSONObject(data);
						} catch(JSONException e) {
							ENV.logger.warning("creat json fail:"+e);
							ps.println("{\"roger\":false}");
							socket.close();
							continue;
						}
						if (obj.getString("op").equals("send result")) {//Operation 1.send result
							if (obj.has("EnvItem") && obj.has("EnvGroup") && obj.has("isFromOwner")) {
								EnvItem ei = new EnvItem(obj.get("EnvItem").toString());
								EnvGroup eg = new EnvGroup(obj.get("EnvGroup").toString());
								...
								ENV.logger.warning("json member error:"+obj.toString());
								ps.println(new JSONStringer().object()
										.key("roger").value(true)
						...
						} else if (obj.getString("op").equals("acquire EnvItem")) {//Operation 2.acquire EnvItem
							if (obj.has("EnvItem") && obj.has("EnvGroup")) {
								EnvItem ei = new EnvItem(obj.get("EnvItem").toString());
								EnvGroup eg = new EnvGroup(obj.get("EnvGroup").toString());
								EnvItem new_ei= tupleSpace.acquireFromPool(ei,eg);
								if (new_ei == null) {
									ps.println(new JSONStringer().object()
											.key("roger").value(true)
									...
									try {
										...
										ENV.logger.warning("can not parse received data into json:"+rcv);
									}
									if (jo.has("roger") && jo.getBoolean("roger")) {					
										...
										ENV.logger.warning("target did not roger that message:"+data);
									}
									socket.close();
									continue;
								}
							} else {//error handle...
								ENV.logger.warning("json member error:"+obj.toString(4));
								ps.println(new JSONStringer().object()
										.key("roger").value(true)
										.key("result").value(false)
										.key("message").value("missing proper key.")
							...
							ENV.logger.warning("Unknown message:" + obj.toString(4));
						}
					}
				} catch (IOException e) {
	...
	public boolean sendDataToNode(String data,String address,int port) {
		if (address.matches("\\d+\\.\\d+\\.\\d+\\.\\d+")) {
			try {
				...
				ps.println(data);
				ENV.logger.info("sending data done:->"+data);
			...
			ENV.logger.warning("IP format not right");
			return false;
		}
	}
	/**
	                ...
	                for (Enumeration<InetAddress> enumIpAddr = intf  
	                    ...
	                    if (!inetAddress.isLoopbackAddress()) {  
	                    	...
	                    	if (ipv4.startsWith("192")){
	                    		ENV.logger.info("local ip:"+ipv4);
	                    		return ipv4;
	                    	}
	                    	else {
	                    		ENV.logger.info("false ip"+ipv4);
	                    		ipv4 = null;
	                    	}
	                    }  
	                }
	 ...
	 * @return
	 */
	public boolean sendResult(EnvItem ei, EnvGroup eg,boolean isFromOwner) {
		String data = new JSONStringer()
		.object()
			.key("op").value("send result")
			.key("EnvItem").value(ei.pack())
			.key("EnvGroup").value(eg.pack())
			...
			try {
				...
				if (jo.has("roger") && jo.getBoolean("roger")) {					
					if (jo.getBoolean("accept")) {
						rtn =  true;
						ENV.logger.info("sending data accept:->"+data);
					} else {
						ENV.logger.info("send data refused:->"+data);
					}
				} else {
					ENV.logger.warning("target did not roger that message:"+data);
				}
	 ...
	 * @param eg
	 */
	public EnvItem acquireEnvItem(EnvItem ei, EnvGroup eg){
		String address = eg.owner.getIP();
		int port = eg.owner.getPort();
		if (address.matches("\\d+\\.\\d+\\.\\d+\\.\\d+")) {
			try {
				...
				if (jo.has("roger") && jo.getBoolean("roger")) {					
					if (jo.getBoolean("result")) {
						EnvItem new_ei = new EnvItem(jo.getString("EnvItem"));
						ENV.logger.info("result:"+new_ei);
						socket.close();
						return new_ei;
					} else {
						ENV.logger.info("result false");
					}
				} else {
					ENV.logger.warning("target did not roger that message:"+data);
				}
